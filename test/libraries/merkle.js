...
contract("Merkle", () => {
  ...
  describe("Height 2 \"all\" permutations", function () {
    ...
    it("Wrong Height [short]; 0-0-0", async () => {
      const merkle = await MerkleWrapper.new()
      await truffleAssert.reverts(merkle.checkMembership(hex_leaves[0], 0, hex_root, hex_proofs[0], 0))
    })

    it("Wrong Height [long]; 1-1-1", async () => {
      const merkle = await MerkleWrapper.new()
      await truffleAssert.reverts(merkle.checkMembership(hex_leaves[1], 1, hex_root, hex_proofs[1], 5))
    })

    it("Wrong Proof length; 0-0-0", async () => {
      const merkle = await MerkleWrapper.new()
      const bad_proof = hex_proofs[0].slice(0, 10)
      await truffleAssert.reverts(merkle.checkMembership(hex_leaves[0], 0, hex_root, bad_proof, 2))
    })
  })
})
