...
contract("Merkle", () => {
  describe("Height 2 all permutations", function() {
    ...
    const concatenated_proofs = proofs.map((pf) =>
      Buffer.concat(pf.map((x) => x.data))
    )
    ...
    it("correct proof at 0", async () => {
      ...
      const res = await merkle.checkMembership(
        hex_leaves[0],
        0,
        hex_root,
        hex_proofs[0],
    ...
    it("Wrong Height [short]; 0-0-0", async () => {
      ...
      await truffleAssert.reverts(
        merkle.checkMembership(hex_leaves[0], 0, hex_root, hex_proofs[0], 0)
      )
    })
    ...
    it("Wrong Height [long]; 1-1-1", async () => {
      const merkle = await MerkleWrapper.new()
      await truffleAssert.reverts(
        merkle.checkMembership(hex_leaves[1], 1, hex_root, hex_proofs[1], 5)
      )
    })
    ...
    it("Wrong Proof length; 0-0-0", async () => {
      ...
      await truffleAssert.reverts(
        merkle.checkMembership(hex_leaves[0], 0, hex_root, bad_proof, 2)
      )
    })
  })
})
